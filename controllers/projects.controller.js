...
export const getProjectsById = async (req, res) => {
  const { id } = req.params; // Assuming the project_id is passed in the URL parameters
  try {
      const Datas = await projects.findOne({ where: { project_id: id } });
...
export const updateProjects = async (req, res) => {
  const { project_id, name, description, img, link } = req.body;
  try {
      const existingData = await projects.findOne({ where: { project_id } });
...
export const deleteProjects = async (req, res) => {
  const { id } = req.params; // Assuming the project_id is passed in the URL parameters
  try {
    const existingData = await projects.findOne({ where: { project_id: id } });
